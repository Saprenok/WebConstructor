...
/**
 * Модуль интерфейса дополнительных параметров
 * PHP version 5.5
 * @category Yii
 */
class AdvancedOptionsMI extends AbstractModelInterface
{
     ...
     * @var string
     */
    public $nameModule = 'AdvancedOptionsMI';
    ...
    {
        return Yii::t('steps', 'Дополнительно');
    }
    ...
    public function JavascriptExperssion()
    {
        ...
        $query .= "
            ...
            var container_Region = '" . (string) Yii::app()->container->Region . "';
            var container_Country = '" . (string) Yii::app()->container->Country . "';
            var container_RegionMoscow = " . (int) Yii::app()->container->RegionMoscow . ";
            var container_RegionEurope = " . (int) Yii::app()->container->RegionEurope . ";
            var container_RegionChina = " . (int) Yii::app()->container->RegionChina . ";
            var container_DilerVersion = " . (int) Yii::app()->container->DilerVersion . ";
            var container_SpecialEdition = " . (int) Yii::app()->container->SpecialEdition . "
            ...
            var form_li_formNak = $('#li_formNak');
            var form_li_formUpak = $('#li_formUpak');
            
            form_li_formNak.hide();

            //Список элементов управления
            var form_input_formNak = $('#" . __CLASS__ . "_formNak');
            var form_input_formUpak = $('#" . __CLASS__ . "_formUpak');
                
            var form_formGateParsing = $('#" . __CLASS__ . "_formGateParsing');
            var form_li_formGateParsing = $('#li_formGateParsing');
            
        ";
        //Присвоение значений из контейнера
        if (Yii::app()->container->formAllAdvancedOptions) { 
            if (Yii::app()->container->Nak) {
                $query .= '$("#' . __CLASS__ . '_formNak").prop("checked", true);';
            } else {
                $query .= '$("#' . __CLASS__ . '_formNak").prop("checked", false);';
            }
            if (Yii::app()->container->Upak) {
                $query .= '$("#' . __CLASS__ . '_formUpak").prop("checked", true);';
            } else {
                $query .= '$("#' . __CLASS__ . '_formUpak").prop("checked", false);';
            }
        }
        
        // Алгоритмы элемента
        $query .= "
            var formCalculateValues = function() {
        ...
        $query .= "
            if (container_TypeF == 'Алюминиевый'.toLowerCase()) {
                if (
                    ...
                    form_li_formNak.show();
                } else {
                    form_li_formNak.hide();
                }
                form_li_formGateParsing.show();
            } else {
                form_formGateParsing.prop('checked',false);
                form_li_formGateParsing.hide();
            }
        ";
