 ...
 */
class GateFrameMI extends AbstractModelInterface
{
    ...
    {
        if ($this->TypeF == "Для секционных ворот" && $this->Bh < 60) {
            return Yii::t("steps", "Ширина должна быть больше 60 мм.");
        } elseif ($this->TypeF != "Для секционных ворот" && $this->Bh < 200) {
            return Yii::t("steps", "Ширина должна быть больше 200 мм.");
        }
        
        if ($this->TypeF == "Панорамная панель" && $this->Hh > 750) {
            return Yii::t("steps", "Высота должна быть меньше 750 мм.");
        }
        
        if ($this->TypeF == "Для секционных ворот" && $this->Hh < 60) {
            return Yii::t("steps", "Высота должна быть больше 60 мм.");
        } elseif ($this->TypeF != "Для секционных ворот" && $this->Hh < 200) {
            return Yii::t("steps", "Высота должна быть больше 200 мм.");
        }
        
        if (Yii::app()->container->CurrentPanelType) {
            $model = ShieldModel::model()->find(array(
                'condition' => 'id=:id',
                'params' => array(
                    ':id' => Yii::app()->container->CurrentPanelType,
                )
            ));
            if (!$model) {
                //  Если используется удалённый щит
                Yii::app()->container->CurrentPanelType = 0;
                Yii::app()->container->typePanel = 2;
                Yii::app()->container->design = -1;
                Yii::app()->container->design2 = -1;
                Yii::app()->container->structure = -1;
                Yii::app()->container->colorOutside = -1;
                Yii::app()->container->colorInside = -1;
                Yii::app()->container->typeSize = -1;
                Yii::app()->container->ShieldPanelSizes = 0;
                return Yii::t('steps', 'Рассчитаного щита уже не существует, необходимо перевыбрать щит!') . "\n" . Yii::t('steps', 'Пройдите по "Далее" с формы "Проем"!');
            }
        }
        
        if ($this->TypeF != "Для секционных ворот" && $this->TypeF != "Панорамная панель") {
            //return Yii::t('steps', 'Данный тип щита находится в разработке!');
        }
    ...
    public function JavascriptExperssion()
    {
        ...
        foreach ($this->elementsVariables as $key => $value) {
            ...
            if ($value['type'] == 'integer') {
                ...
                if ($value['svg'] == 'true') {
                    $input .= 'svgInfo("' . $key . '","aperture",this);';
        ...
        foreach ($experssions as $key => $value) {
            foreach ($value as $event => $function) {
                if (!empty($function)) {
                    $query .= 'jQuery("#' . __CLASS__ . '_' . $key . '").live("' . $event . '", function(){ ' . $function . '});';
                }
            }
        }
        $query .= "
            ...
            var form_TypeF = $('#" . __CLASS__ . "_TypeF');
            $('#" . __CLASS__ . "_TypeF option:first').remove();
        ";
        ...
        //var_dump(Yii::app()->container->SelectedSizes);
        $query .= "var reCallProfile = false;";
        $query .= "var globalError = null;";

        $query.=$this->getTooltip('GateFrameMI_Bh', '1');
        $query.=$this->getTooltip('GateFrameMI_Hh', '2');
        $query.=$this->getTooltip('GateFrameMI_TypeF', '3');
